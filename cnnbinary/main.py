...
def model_fit(model,train_generator,validation_generator,epochs=1):
    ...
    modelCheckpooint = callbacks.ModelCheckpoint("model_b_checkpoint.h5".format(MODEL), monitor="val_loss", verbose=0, save_best_only=True)
...
def sv_model(model,history,filename):
    np.save('model_b_history.npy',history.history)
